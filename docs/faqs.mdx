...
### **What kind of permissions can be described on a session?**
    ...
    • We not necessarily have too prove consistency of the transaction with the complete set of permissions, but only on the ones the transaction actually interact with.
...
### **What is session composition?**
    ...
    Certain use cases require session keys with broader permissions; however, optimal security is achieved when scopes are small and simple. **Session composition** enables independent sessions to combine and  constraint their permissions with each other’s, allowing users to define complex on-chain behaviors without granting excessive permissions to a single key: Security through simplicity, complexity through composition.
...
### **What can an AI agent providers achieve by partnering?**
...
### **What exactly does your traction (number of light nodes) actually mean?**
    ...
    In that sense, our traction mean two things:
